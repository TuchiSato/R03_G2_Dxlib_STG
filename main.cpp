...
BOOL GameLoad(VOID)
{
	...
	//画像を分割して読み込み
	if (LoadImageDivMen(&tama_moto.handle[0], tama_moto.path, tama_moto.DivYoko, tama_moto.DivTate) == FALSE) { return FALSE; }
...
VOID PlayDraw(VOID)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		{
			back[i].y = -back[i].height +3;	//高さ分、上に戻す
		}
